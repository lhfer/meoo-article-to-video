"""One-time verified import of the supplied Meoo brand assets."""
...
assert sha(logo)==EXPECTED_LOGO,'Logo checksum mismatch'
(OUT/'meoo-logo.png').write_bytes(logo)
print('Verified wordmark:',sha(logo))
...
for n in range(1,5):
    ...
    try:
        ...
        if sha(data)==EXPECTED_GIF:
            (OUT/'meoo-mascot.gif').write_bytes(data);break
    except Exception as e:print('Fetch failed',n,type(e).__name__)
mascot=OUT/'meoo-mascot.gif'
assert mascot.is_file(),'The supplied mascot could not be imported with a matching checksum'
...
print('All brand assets verified.')
